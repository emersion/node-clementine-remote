	...
	this.on('message', function (msg) {
		...
		switch (msg.type) {
			case MsgType.CONNECT:
				...
				if (server.options.auth_code) { //Check auth code
					if (req.auth_code !== server.options.auth_code) {
						that.write(builder.disconnect('Wrong_Auth_Code'));
						that.end();
						return;
					}
				}
				...
				if (req.send_playlist_songs) {
					var activePlaylist = server.getPlaylist();
					if (activePlaylist) {
						that.write(builder.playlist_songs(activePlaylist));
					}
				}
			...
			case MsgType.REQUEST_PLAYLIST_SONGS:
				var playlist = server.getPlaylist(msg.request_playlist_songs.id);
				if (!playlist) return;

				that.write(builder.playlist_songs(playlist));
				break;
			...
			case MsgType.LOVE: // Last.fm
			case MsgType.BAN: // Last.fm
...
ServerConnection.prototype.write = function (msg) {
	...
	return Connection.prototype.write.call(this, msg);
};

ServerConnection.prototype.end = function () {
	return Connection.prototype.end.call(this);
};
...
function Server(opts) {
	...
	Object.defineProperties(this, {
		...
		activePlaylist: {
			get: function () {
				return props.activePlaylist;
			},
			set: function (playlistId) {
				if (!that.getPlaylist(playlistId)) return;

				props.activePlaylist = playlistId;
				that.broadcast(that.builder.change_song(playlistId, 0));
			}
		}
	});
	...
	this.playlists = new EventedArray();

	//var activePlaylist = new Playlist('Now playing');
	...
	this.playlists.on('add', function (playlist) {
		playlist.on('update', function () {
			that.broadcast(that.builder.playlist_songs(playlist));
		});
	});

	this.playlists.on('update', function () {
		that.broadcast(that.builder.playlists());
	});
...
Server.prototype.getPlaylist = function (playlistId) {
	if (typeof playlistId == 'undefined') {
		playlistId = this.activePlaylist;
	}
...
Server.prototype.close = function (done) {
	...
	this.broadcast(this.builder.disconnect('Server_Shutdown'));
